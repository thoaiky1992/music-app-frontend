        ...
        <SwiperSlide key={song._id}>
          <div className="flex flex-col relative group cursor-pointer">
            ...
            <figure className="relative z-0 h-[150px] md:h-[200px]">
              {playListStore.list[playListStore.index]._id === song._id && playListStore.isPlay && (
                <div
                  className={classNames("absolute w-full h-full flex justify-center items-center group-hover:hidden")}
                >
                  <img src={IconPlayingGif} className="w-8 h-8" alt="" />
                </div>
              )}
              <img src={song.image} alt="" className="h-[150px] md:h-[200px] w-full object-cover rounded-lg" />
            </figure>
